...
Output: 31 bands aligned to NTIRE standard (400–700 nm, 10 nm intervals).

Band selection: 31 indices picked from the 176-band kejian camera
(402.6–1005.5 nm) by nearest-wavelength matching. Max error: 2.6 nm.

Usage:
    python train_31.py
    python train_31.py --epochs 300 --batch_size 8 --lr 4e-4
"""
...
CKPT_DIR    = Path(r'E:\2026\05\r2h\checkpoints\v3_31bands')
LOG_PATH    = Path(r'E:\2026\05\r2h\train_log_31.csv')
HSI_BANDS   = 31
EPS         = 1e-3   # MRAE epsilon

# 31 band indices from the 176-band wavelength array (402.6–1005.5 nm)
# corresponding to NTIRE targets: 400, 410, 420, ..., 700 nm (10 nm step)
...
def make_mask(gt: torch.Tensor, thresh: float) -> torch.Tensor:
    ...
    return torch.clamp(gt_mean / thresh, 0.0, 1.0)


def mrae_loss(pred: torch.Tensor, gt: torch.Tensor,
              mask: torch.Tensor = None) -> torch.Tensor:
    per_pixel = torch.abs(pred - gt) / (gt + EPS)
    if mask is not None:
...
def parse_args():
    ...
    p.add_argument('--pretrain',   type=str,   default=None,
                   help='path to pretrained weights (strict=False, shape-matched)')
...
def main():
    ...
    print(f'Output bands: {HSI_BANDS}  (NTIRE 400–700 nm, 10 nm step)')

    train_loader, val_loader, _ = get_dataloaders(
        PATCH_DIR, batch_size=args.batch_size,
        num_workers=args.num_workers, band_indices=BAND_INDICES)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss = train_one_epoch(
            model, train_loader, optimizer, device, epoch, args.epochs, args.mask_thresh)
